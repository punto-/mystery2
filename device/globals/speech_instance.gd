...
func input(event):
	if event.is_action("use") && event.is_pressed() && !event.is_echo():
		skipped()
...
func init(p_params, p_context):
	character = vm.game.get_object(p_params[0])
	context = p_context
	text = p_params[1]
...
func _ready():
	...
	finish_with_speech = Globals.get("application/skip_text_after_speech")
